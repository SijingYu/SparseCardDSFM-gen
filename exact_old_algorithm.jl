...
function algorithm(A::SparseMatrixCSC{Float64,Int64}, to_exclude::Vector{Int64}, start_beta::Float64, convex_f)
    N = size(A,1)
    r = length(to_exclude)
    ak_s = a_k_s(N-r, convex_f)
    ...
    new_beta = cut/convex_f(Float64(length(S)))
    ...
    while new_beta < beta
        ...
        #println(length(S))
        new_beta = cut_S(A, S)/convex_f(Float64(length(S)))
        #=
        println("=================================")
        println("New set length with beta "*string(beta))
        println(length(S))
        println("New beta updated with the set")
        println(new_beta)=#
       #break
    end
    return beta, S_best
...
function faster_st_cut_step_exclude_R_a_k(A::SparseMatrixCSC{Float64,Int64}, to_exclude::Vector{Int64}, a_ks::Vector{Float64}, beta::Float64)
    ...
    @time ak_matrix = aux_sub_construction(N, beta_aks, to_exclude)
...
function aux_sub_construction(N, a_ks::Vector{Float64}, to_exclude)
    ...
    m = N
    n = N-q-1
    return @time SparseMatrixCSC(m,n,colp,rowv,nzv)
end
