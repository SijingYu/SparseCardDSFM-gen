...
function DSP_approx_gen_dense_sub(G::SparseMatrixCSC{Float64,Int64}, epsilon::Float64, exponent::Float64, to_exclude::Vector{Int64})
    ...
    while betaBest < beta_old
        for i in 1:(N+1)
            EdgesW[1][i] = betaBest*(concave_f(i-1, exponent))
        end
        ...
        A[1:N,1:N] =  A[1:N,1:N] + (betaBest/2).*G
        s_red = s_red + d_G
        t_red = t_red + (1-betaBest/2).*d_G
...
function DSGScore(G::SparseMatrixCSC{Float64,Int64}, S::Vector{Int64}, alpha::Float64)
    ...
    N = size(G)[1]
    s_len = length(S)
    inner_edges = (vol-cut)/2

    return concave_f(s_len, alpha)/inner_edges
end
...
function concave_f(x::Int64, a::Float64)
    return x^a
end
