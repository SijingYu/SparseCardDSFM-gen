...
function maxflow(A::Union{SparseMatrixCSC,MatrixNetwork},svec::Vector{Float64},tvec::Vector{Float64}, flowtol::Union{Float64,Int}= 1e-6)
    ...
    println("ExcessNodes done")
    t2 = time() - t1
    println("Time used = $t2")
    ...
    println("Main Push Relabel done")
    t3 = time() - t2
    println("Time used = $t3")
...
function Main_Push_Relabel(C::SparseMatrixCSC,
    ...
    n = size(C,1)
    println("Start Main Push Relabel")
    start = time()
    ...
    println("Storing adjacency list done")
    t1 = time()-start
    println(t1)
    ...
    t3= time()-t2
    println("Relabeling_bfs takes")
    println(t3)

    println("Start working on active nodes")
    ...
    t5= time()-t4
    println("Relabeling_bf again takes takes")
    println(t5)
